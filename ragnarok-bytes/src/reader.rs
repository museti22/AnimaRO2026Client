    ...
    Meta: Any + 'static,
{
    ...
    /// Returns a copy of the remaining bytes without advancing the reader.
    pub fn peek_remaining(&self) -> Vec<u8> {
        self.data[self.offset..self.limit].to_vec()
    }

    /// Advance the reader by `count` bytes, skipping over them.
    /// Returns an error if there aren't enough bytes remaining.
