...
pub enum HandlerResult<Output> {
    ...
    /// Contains the unrecognized packet header for logging/skip purposes.
    UnhandledPacket(PacketHeader),
    /// Packet was most likely cut-off.
    PacketCutOff,
    /// An error occurred inside the packet handler.
    /// Contains the packet header that failed to parse.
    ...
    Callback: PacketCallback,
{
    ...
    pub fn process_one(&mut self, byte_reader: &mut ByteReader<Meta>) -> HandlerResult<Output> {
        ...
        let Some(handler) = self.handlers.get(&header) else {
            ...
            // Note: unknown_packet consumes remaining_bytes, so we clone the
            // data before calling it to avoid losing the buffer contents.
            let remaining = byte_reader.peek_remaining();
            self.packet_callback.unknown_packet(remaining);

            return HandlerResult::UnhandledPacket(header);
        };
        ...
        match handler(byte_reader) {
            ...
            Err(error) => {
                ...
                let remaining = byte_reader.peek_remaining();
                self.packet_callback.failed_packet(remaining, error.clone());
